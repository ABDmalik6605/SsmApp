...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM1581constructorimpl;
        boolean z;
            ...
            while (true) {
                if (segment2.getId() < j || segment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM1581constructorimpl = SegmentOrClosed.m1581constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM1581constructorimpl = SegmentOrClosed.m1581constructorimpl(segment2);
                    break;
                }
            }
            if (SegmentOrClosed.m1586isClosedimpl(objM1581constructorimpl)) {
                break;
            }
            Segment segmentM1584getSegmentimpl = SegmentOrClosed.m1584getSegmentimpl(objM1581constructorimpl);
            while (true) {
                Segment segment3 = (Segment) this.tail;
                if (segment3.getId() >= segmentM1584getSegmentimpl.getId()) {
                    break;
                }
                if (!segmentM1584getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    break;
                }
                if (tail$FU.compareAndSet(this, segment3, segmentM1584getSegmentimpl)) {
                    if (segment3.decPointers$kotlinx_coroutines_core()) {
                        segment3.remove();
                    }
                } else if (segmentM1584getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    segmentM1584getSegmentimpl.remove();
                }
            }
            z = true;
        } while (!z);
        SemaphoreSegment semaphoreSegment = (SemaphoreSegment) SegmentOrClosed.m1584getSegmentimpl(objM1581constructorimpl);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        do {
            ...
            while (true) {
                ...
                if (head$FU.compareAndSet(this, segment3, segmentM1584getSegmentimpl)) {
                    if (segment3.decPointers$kotlinx_coroutines_core()) {
                        segment3.remove();
                    }
                } else if (segmentM1584getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
