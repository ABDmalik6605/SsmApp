...
public final class SelectInstance<R> extends LockFreeLinkedListHead implements SelectBuilder<R>, kotlinx.coroutines.selects.SelectInstance<R>, Continuation<R>, CoroutineStackFrame {
    ...
    public final void handleBuilderException(Throwable e) {
        if (trySelect()) {
            ...
            resumeWith(Result.m77constructorimpl(ResultKt.createFailure(e)));
        } else {
            ...
            if (obj == SelectKt.UNDECIDED) {
                ...
                if (_result$FU.compareAndSet(this, IntrinsicsKt.getCOROUTINE_SUSPENDED(), SelectKt.RESUMED)) {
                    if (Result.m83isFailureimpl(result)) {
                        Continuation<R> continuation = this.uCont;
                        Throwable thM80exceptionOrNullimpl = Result.m80exceptionOrNullimpl(result);
                        Intrinsics.checkNotNull(thM80exceptionOrNullimpl);
                        Result.Companion companion = Result.INSTANCE;
                        if (DebugKt.getRECOVER_STACK_TRACES() && (continuation instanceof CoroutineStackFrame)) {
                            thM80exceptionOrNullimpl = StackTraceRecoveryKt.recoverFromStackFrame(thM80exceptionOrNullimpl, (CoroutineStackFrame) continuation);
                        }
                        continuation.resumeWith(Result.m77constructorimpl(ResultKt.createFailure(thM80exceptionOrNullimpl)));
                        return;
                    }
        ...
        while (true) {
            ...
            if (obj == SelectKt.UNDECIDED) {
                ...
                if (_result$FU.compareAndSet(this, IntrinsicsKt.getCOROUTINE_SUSPENDED(), SelectKt.RESUMED)) {
                    ...
                    Result.Companion companion = Result.INSTANCE;
                    continuationIntercepted.resumeWith(Result.m77constructorimpl(ResultKt.createFailure(exception)));
                    return;
                }
            } else {
                throw new IllegalStateException("Already resumed");
