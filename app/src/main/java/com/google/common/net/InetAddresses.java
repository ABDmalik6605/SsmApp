...
public final class InetAddresses {
    ...
    private static byte[] textToNumericFormatV6(String str) {
        ...
        int i = -1;
        for (int i2 = 1; i2 < strArrSplit.length - 1; i2++) {
        ...
        if (i >= 0) {
            length2 = (strArrSplit.length - i) - 1;
        ...
        for (int i4 = 0; i4 < length; i4++) {
            try {
                byteBufferAllocate.putShort(parseHextet(strArrSplit[i4]));
            } catch (NumberFormatException unused) {
                return null;
            }
        }
    ...
    private static byte parseOctet(String str) throws NumberFormatException {
        ...
        if (i > 255 || (str.startsWith(Constant.ECE_Katchi) && str.length() > 1)) {
