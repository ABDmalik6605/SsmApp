...
final class Field25519 {
    ...
    static void reduceCoefficients(long[] output) {
        ...
        int i = 0;
        while (i < 10) {
            long j = output[i] / 67108864;
            output[i] = output[i] - (j << 26);
            int i2 = i + 1;
            output[i2] = output[i2] + j;
            long j2 = output[i2] / 33554432;
    ...
    static byte[] contract(long[] inputLimbs) {
        ...
        for (int i = 0; i < 2; i++) {
            int i2 = 0;
            while (i2 < 9) {
                int i3 = -((int) ((jArrCopyOf[i2] & (jArrCopyOf[i2] >> 31)) >> SHIFT[i2 & 1]));
                jArrCopyOf[i2] = jArrCopyOf[i2] + (i3 << r9[r10]);
                i2++;
                jArrCopyOf[i2] = jArrCopyOf[i2] - i3;
            }
            int i4 = -((int) (((jArrCopyOf[9] >> 31) & jArrCopyOf[9]) >> 25));
            jArrCopyOf[9] = jArrCopyOf[9] + (i4 << 25);
            jArrCopyOf[0] = jArrCopyOf[0] - (i4 * 19);
        }
        int i5 = -((int) ((jArrCopyOf[0] & (jArrCopyOf[0] >> 31)) >> 26));
        jArrCopyOf[0] = jArrCopyOf[0] + (i5 << 26);
        jArrCopyOf[1] = jArrCopyOf[1] - i5;
        for (int i6 = 0; i6 < 2; i6++) {
            int i7 = 0;
            while (i7 < 9) {
                int i8 = (int) (jArrCopyOf[i7] >> SHIFT[i7 & 1]);
                jArrCopyOf[i7] = MASK[r11] & jArrCopyOf[i7];
                i7++;
                jArrCopyOf[i7] = jArrCopyOf[i7] + i8;
            }
        }
        int i9 = (int) (jArrCopyOf[9] >> 25);
        jArrCopyOf[9] = jArrCopyOf[9] & 33554431;
        jArrCopyOf[0] = jArrCopyOf[0] + (i9 * 19);
        int iGte = gte((int) jArrCopyOf[0], 67108845);
        for (int i10 = 1; i10 < 10; i10++) {
            iGte &= eq((int) jArrCopyOf[i10], MASK[i10 & 1]);
        }
        jArrCopyOf[0] = jArrCopyOf[0] - (67108845 & iGte);
        long j = 33554431 & iGte;
        jArrCopyOf[1] = jArrCopyOf[1] - j;
        for (int i11 = 2; i11 < 10; i11 += 2) {
            jArrCopyOf[i11] = jArrCopyOf[i11] - (67108863 & iGte);
            int i12 = i11 + 1;
            jArrCopyOf[i12] = jArrCopyOf[i12] - j;
        }
        for (int i13 = 0; i13 < 10; i13++) {
            jArrCopyOf[i13] = jArrCopyOf[i13] << EXPAND_SHIFT[i13];
        }
        byte[] bArr = new byte[32];
        for (int i14 = 0; i14 < 10; i14++) {
            int[] iArr = EXPAND_START;
            bArr[iArr[i14]] = (byte) (bArr[r6] | (jArrCopyOf[i14] & 255));
            bArr[iArr[i14] + 1] = (byte) (bArr[r6] | ((jArrCopyOf[i14] >> 8) & 255));
            bArr[iArr[i14] + 2] = (byte) (bArr[r6] | ((jArrCopyOf[i14] >> 16) & 255));
            bArr[iArr[i14] + 3] = (byte) (bArr[r3] | ((jArrCopyOf[i14] >> 24) & 255));
        }
        return bArr;
    }
