...
public final class AnimatorSet extends Animator {
    ...
    @Override // com.nineoldandroids.animation.Animator
    /* renamed from: clone */
    public AnimatorSet mo67clone() {
        AnimatorSet animatorSet = (AnimatorSet) super.mo67clone();
        ...
        while (it.hasNext()) {
            Node next = it.next();
            Node nodeM68clone = next.m68clone();
            map.put(next, nodeM68clone);
            animatorSet.mNodes.add(nodeM68clone);
            animatorSet.mNodeMap.put(nodeM68clone.animation, nodeM68clone);
            ArrayList arrayList = null;
            nodeM68clone.dependencies = null;
            nodeM68clone.tmpDependencies = null;
            nodeM68clone.nodeDependents = null;
            nodeM68clone.nodeDependencies = null;
            ArrayList<Animator.AnimatorListener> listeners = nodeM68clone.animation.getListeners();
            if (listeners != null) {
    ...
    private static class Node implements Cloneable {
        ...
        public Node m68clone() {
            try {
                Node node = (Node) super.clone();
                node.animation = this.animation.mo67clone();
                return node;
