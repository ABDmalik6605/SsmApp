...
public abstract class BaseContinuationImpl implements Continuation<Object>, CoroutineStackFrame, Serializable {
    ...
    public final void resumeWith(Object result) {
        ...
        while (true) {
            ...
            Continuation completion = baseContinuationImpl.getCompletion();
            Intrinsics.checkNotNull(completion);
            try {
                objInvokeSuspend = baseContinuationImpl.invokeSuspend(result);
            } catch (Throwable th) {
                Result.Companion companion = Result.INSTANCE;
                result = Result.m77constructorimpl(ResultKt.createFailure(th));
            }
            if (objInvokeSuspend == IntrinsicsKt.getCOROUTINE_SUSPENDED()) {
                return;
            }
            Result.Companion companion2 = Result.INSTANCE;
            result = Result.m77constructorimpl(objInvokeSuspend);
