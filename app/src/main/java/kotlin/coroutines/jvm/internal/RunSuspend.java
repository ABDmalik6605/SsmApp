...
final class RunSuspend implements Continuation<Unit> {
    ...
    public final Result<Unit> m1264getResultxLWZpok() {
        return this.result;
    }
    ...
    public void resumeWith(Object result) {
        synchronized (this) {
            setResult(Result.m76boximpl(result));
            notifyAll();
    ...
    public final void await() {
        synchronized (this) {
            while (true) {
                Result<Unit> resultM1264getResultxLWZpok = m1264getResultxLWZpok();
                if (resultM1264getResultxLWZpok == null) {
                    wait();
                } else {
                    ResultKt.throwOnFailure(resultM1264getResultxLWZpok.getValue());
                }
            }
        }
    }
